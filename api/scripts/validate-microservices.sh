...
if grep -q "PAYMENT_SERVICE" wrangler-gateway.toml && grep -q "BASE_SERVICE" wrangler-gateway.toml; then
    echo -e "${GREEN}[OK]${NC} Gateway has service bindings configured"
else
    echo -e "${RED}[FAIL]${NC} Gateway missing service bindings"
    errors=$((errors+1))
fi

# Check that payment routes are removed from app.js
if ! grep -q "require('./api/routes/payments')" app.js; then
    echo -e "${GREEN}[OK]${NC} Payment routes removed from app.js"
else
    echo -e "${RED}[FAIL]${NC} Payment routes still in app.js"
    errors=$((errors+1))
fi

if ! grep -q "app.use('/payments'" app.js; then
    echo -e "${GREEN}[OK]${NC} Payment route mounting removed from app.js"
else
    echo -e "${RED}[FAIL]${NC} Payment route mounting still in app.js"
    errors=$((errors+1))
fi

# Check that gateway worker routes correctly
if grep -q "/api/payments" src/gateway-worker.js; then
    echo -e "${GREEN}[OK]${NC} Gateway routes /api/payments"
else
    echo -e "${RED}[FAIL]${NC} Gateway doesn't route /api/payments"
    errors=$((errors+1))
fi

# Check that payment worker has payment routes
if grep -q "paymentRoutes" src/payment-worker.js; then
    echo -e "${GREEN}[OK]${NC} Payment worker imports payment routes"
else
    echo -e "${RED}[FAIL]${NC} Payment worker missing payment routes"
    errors=$((errors+1))
fi

# Check package.json has deployment scripts
if grep -q "deploy:all" package.json && grep -q "deploy:gateway" package.json; then
    echo -e "${GREEN}[OK]${NC} Deployment scripts added to package.json"
else
    echo -e "${RED}[FAIL]${NC} Deployment scripts missing from package.json"
    errors=$((errors+1))
fi
