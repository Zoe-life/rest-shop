...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Security Note: Payment gateway secrets should be configured in the
      // backend environment directly, not forwarded from workers. The backend
      // should read from its own environment variables. Workers only handle routing.
      // Forwarding secrets via headers creates security risks (interception, logging).
